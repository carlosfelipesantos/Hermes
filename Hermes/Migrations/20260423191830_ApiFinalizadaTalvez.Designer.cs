// <auto-generated />
using System;
using Hermes.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace Hermes.Migrations
{
    [DbContext(typeof(HermesBD))]
    [Migration("20260423191830_ApiFinalizadaTalvez")]
    partial class ApiFinalizadaTalvez
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.10")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("Hermes.Entities.Avaliacao", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("ClienteId")
                        .HasColumnType("int");

                    b.Property<string>("Comentario")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("DataAvaliacao")
                        .HasColumnType("datetime2");

                    b.Property<int>("FreteId")
                        .HasColumnType("int");

                    b.Property<decimal>("Nota")
                        .HasColumnType("decimal(3,2)");

                    b.Property<int>("TransportadorId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ClienteId");

                    b.HasIndex("FreteId")
                        .IsUnique();

                    b.HasIndex("TransportadorId")
                        .HasDatabaseName("IX_Avaliacoes_TransportadorId");

                    b.ToTable("Avaliacoes");
                });

            modelBuilder.Entity("Hermes.Entities.DisponibilidadeBase", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("DiaSemana")
                        .HasColumnType("int");

                    b.Property<TimeSpan>("HoraFim")
                        .HasColumnType("time");

                    b.Property<TimeSpan>("HoraInicio")
                        .HasColumnType("time");

                    b.Property<int>("TransportadorId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("TransportadorId", "DiaSemana")
                        .HasDatabaseName("IX_DisponibilidadesBase_TransportadorId_DiaSemana");

                    b.ToTable("DisponibilidadesBase");
                });

            modelBuilder.Entity("Hermes.Entities.Frete", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("BairroDestino")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BairroOrigem")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CidadeDestino")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CidadeOrigem")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("ClienteId")
                        .HasColumnType("int");

                    b.Property<string>("DDDOrigem")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("DataConclusao")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("DataHoraFimPrevisto")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DataHoraFimReal")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("DataHoraInicio")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("DataSolicitacao")
                        .HasColumnType("datetime2");

                    b.Property<string>("DescricaoCarga")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("DescricaoDestino")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("DescricaoOrigem")
                        .HasColumnType("nvarchar(max)");

                    b.Property<double?>("DistanciaExtra")
                        .HasColumnType("float");

                    b.Property<TimeSpan>("DuracaoEstimada")
                        .HasColumnType("time");

                    b.Property<string>("EstadoDestino")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("EstadoOrigem")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<double?>("LatitudeDestino")
                        .HasColumnType("float");

                    b.Property<double?>("LatitudeOrigem")
                        .HasColumnType("float");

                    b.Property<double?>("LongitudeDestino")
                        .HasColumnType("float");

                    b.Property<double?>("LongitudeOrigem")
                        .HasColumnType("float");

                    b.Property<bool>("SitioDestino")
                        .HasColumnType("bit");

                    b.Property<bool>("SitioOrigem")
                        .HasColumnType("bit");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<int>("TipoCarga")
                        .HasColumnType("int");

                    b.Property<int?>("TransportadorId")
                        .HasColumnType("int");

                    b.Property<bool>("Urgente")
                        .HasColumnType("bit");

                    b.Property<decimal>("Valor")
                        .HasColumnType("decimal(10,2)");

                    b.HasKey("Id");

                    b.HasIndex("CidadeOrigem")
                        .HasDatabaseName("IX_Fretes_CidadeOrigem");

                    b.HasIndex("ClienteId");

                    b.HasIndex("Status", "DataSolicitacao")
                        .HasDatabaseName("IX_Fretes_Status_DataSolicitacao");

                    b.HasIndex("TransportadorId", "Status")
                        .HasDatabaseName("IX_Fretes_TransportadorId_Status");

                    b.ToTable("Fretes");
                });

            modelBuilder.Entity("Hermes.Entities.Notificacao", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime2");

                    b.Property<int?>("FreteId")
                        .HasColumnType("int");

                    b.Property<bool>("Lida")
                        .HasColumnType("bit");

                    b.Property<string>("Mensagem")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Tipo")
                        .HasColumnType("int");

                    b.Property<string>("Titulo")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("UsuarioId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("FreteId");

                    b.HasIndex("UsuarioId", "DataCriacao")
                        .HasDatabaseName("IX_Notificacoes_UsuarioId_DataCriacao");

                    b.ToTable("Notificacoes");
                });

            modelBuilder.Entity("Hermes.Entities.Usuario", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<bool>("Ativo")
                        .HasColumnType("bit");

                    b.Property<string>("Cidade")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("DDD")
                        .IsRequired()
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<DateTime>("DataCadastro")
                        .HasColumnType("datetime2");

                    b.Property<string>("Discriminator")
                        .IsRequired()
                        .HasMaxLength(13)
                        .HasColumnType("nvarchar(13)");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Endereco")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FotoPerfil")
                        .HasColumnType("nvarchar(max)");

                    b.Property<double?>("Latitude")
                        .HasColumnType("float");

                    b.Property<double?>("Longitude")
                        .HasColumnType("float");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Senha")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Telefone")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<int>("Tipo")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("Email")
                        .IsUnique();

                    b.HasIndex("DDD", "Telefone")
                        .IsUnique();

                    b.ToTable("Usuarios");

                    b.HasDiscriminator().HasValue("Usuario");

                    b.UseTphMappingStrategy();
                });

            modelBuilder.Entity("Hermes.Entities.Veiculo", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Capacidade")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("DataCadastro")
                        .HasColumnType("datetime2");

                    b.Property<bool>("Disponivel")
                        .HasColumnType("bit");

                    b.Property<string>("Marca")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Modelo")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Placa")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("TipoVeiculo")
                        .HasColumnType("int");

                    b.Property<int>("TransportadorId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("TransportadorId", "TipoVeiculo")
                        .HasDatabaseName("IX_Veiculos_TransportadorId_TipoVeiculo");

                    b.ToTable("Veiculos");
                });

            modelBuilder.Entity("Hermes.Entities.Cliente", b =>
                {
                    b.HasBaseType("Hermes.Entities.Usuario");

                    b.Property<DateTime?>("DataNasc")
                        .HasColumnType("datetime2");

                    b.HasDiscriminator().HasValue("Cliente");
                });

            modelBuilder.Entity("Hermes.Entities.Transportador", b =>
                {
                    b.HasBaseType("Hermes.Entities.Usuario");

                    b.Property<double?>("AvaliacaoMedia")
                        .HasColumnType("float");

                    b.Property<bool>("Disponivel")
                        .HasColumnType("bit");

                    b.Property<string>("Documento")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("TotalAvaliacoes")
                        .HasColumnType("int");

                    b.HasDiscriminator().HasValue("Transportador");
                });

            modelBuilder.Entity("Hermes.Entities.Avaliacao", b =>
                {
                    b.HasOne("Hermes.Entities.Cliente", "Cliente")
                        .WithMany()
                        .HasForeignKey("ClienteId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Hermes.Entities.Frete", "Frete")
                        .WithOne("Avaliacao")
                        .HasForeignKey("Hermes.Entities.Avaliacao", "FreteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Hermes.Entities.Transportador", "Transportador")
                        .WithMany()
                        .HasForeignKey("TransportadorId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Cliente");

                    b.Navigation("Frete");

                    b.Navigation("Transportador");
                });

            modelBuilder.Entity("Hermes.Entities.DisponibilidadeBase", b =>
                {
                    b.HasOne("Hermes.Entities.Transportador", "Transportador")
                        .WithMany("Disponibilidades")
                        .HasForeignKey("TransportadorId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Transportador");
                });

            modelBuilder.Entity("Hermes.Entities.Frete", b =>
                {
                    b.HasOne("Hermes.Entities.Cliente", "Cliente")
                        .WithMany("FretesSolicitados")
                        .HasForeignKey("ClienteId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Hermes.Entities.Transportador", "Transportador")
                        .WithMany("FretesAceitos")
                        .HasForeignKey("TransportadorId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Cliente");

                    b.Navigation("Transportador");
                });

            modelBuilder.Entity("Hermes.Entities.Notificacao", b =>
                {
                    b.HasOne("Hermes.Entities.Frete", "Frete")
                        .WithMany()
                        .HasForeignKey("FreteId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Hermes.Entities.Usuario", "Usuario")
                        .WithMany("Notificacoes")
                        .HasForeignKey("UsuarioId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Frete");

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("Hermes.Entities.Veiculo", b =>
                {
                    b.HasOne("Hermes.Entities.Transportador", "Transportador")
                        .WithMany("Veiculos")
                        .HasForeignKey("TransportadorId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Transportador");
                });

            modelBuilder.Entity("Hermes.Entities.Frete", b =>
                {
                    b.Navigation("Avaliacao");
                });

            modelBuilder.Entity("Hermes.Entities.Usuario", b =>
                {
                    b.Navigation("Notificacoes");
                });

            modelBuilder.Entity("Hermes.Entities.Cliente", b =>
                {
                    b.Navigation("FretesSolicitados");
                });

            modelBuilder.Entity("Hermes.Entities.Transportador", b =>
                {
                    b.Navigation("Disponibilidades");

                    b.Navigation("FretesAceitos");

                    b.Navigation("Veiculos");
                });
#pragma warning restore 612, 618
        }
    }
}
